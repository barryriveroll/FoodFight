...
$(document).ready(function() {
  ...
  socket.on("startGame", function(playerData) {
    ...
    $.get("/api/board", function(data) {
      for (var i = 0; i < playerData.length - 1; i++) {
        ...
        newPlayerDiv
          .addClass("player" + playerData[2][i] + "card")
          .addClass("player-card")
          .attr("id", "player-card-" + playerData[2][i]);

        if (playerData[2][i] == playerNum) {
          newPlayerDiv.append("<div class='your-move-div'>YOU!</div>");
        }
        newPlayerDiv.append("<h5>Player " + playerData[2][i] + "</h5>");
        newPlayerDiv.append(
        ...
        if (i === 0) {
          ...
          $("#player-card-" + playerData[2][i]).css("height", "210px");
      ...
      for (var i = 0; i < board.spots.length; i++) {
        ...
        if (board.spots[i].hasPlayer) {
          ...
          if (board.spots[i].playerId === parseInt(playerData[2][0])) {
      ...
      changeTurn(parseInt(playerData[2][0]), boardSpot, false);
    });
  });
  ...
  socket.on("startBattleTurn", function(data) {
    ...
    var hpValues = [];
    hpValues.push(players.p1.hp, players.p2.hp);
